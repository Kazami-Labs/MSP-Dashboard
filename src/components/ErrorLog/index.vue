<template>
  <div v-if="errorLogs.length>0">
    ...
    <el-dialog :visible.sync="dialogTableVisible" :title="$t('errorLogView.title')" width="80%">
      <el-table :data="errorLogs" border>
        <el-table-column :label="$t('errorLogView.detail')">
          <template slot-scope="scope">
            <div>
              <span class="message-title">{{ $t('errorLogView.msg') }}</span>
              <el-tag type="danger">{{ scope.row.err.message }}</el-tag>
            </div>
            <br>
            <div>
              <span class="message-title" style="padding-right: 10px;">{{ $t('errorLogView.info') }}</span>
              <el-tag
                type="warning"
              >{{ scope.row.vm.$vnode.tag }} {{ $t('errorLogView.in') }} {{ scope.row.info }}</el-tag>
            </div>
            <br>
            <div>
              <span class="message-title" style="padding-right: 16px;">{{ $t('errorLogView.url') }}</span>
              <el-tag type="success">{{ scope.row.url }}</el-tag>
            </div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('errorLogView.stack')">
